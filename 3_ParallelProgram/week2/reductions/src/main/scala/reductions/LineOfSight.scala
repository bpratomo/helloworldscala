...
object LineOfSight extends LineOfSightInterface:
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit =
    var m:Float = Float.MinValue
    for (i <- 0 until input.length){
      val c = if i == 0 then 0 else input(i)/i
      m = m max c
      output(i) = m 
    }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    tree: Tree): Unit =
    tree match {
      case Tree.Leaf(from,end,maxPrevious) => downsweepSequential(input,output,startingAngle,from,end)
      case Tree.Node(left,right) =>  parallel(downsweep(input,output,startingAngle,left),downsweep(input,output,left.maxPrevious,right))
    }
