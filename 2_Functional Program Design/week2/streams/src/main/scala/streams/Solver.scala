...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(
    ...
    for (block, move) <- b.legalNeighbors.to(LazyList)
    yield {
      println(s"block is $block,move is $move, and history is $history");
      (block, move :: history)
    }
  ...
  def from(
    ...
    if frontier.isEmpty then { println(initial); initial }
    else
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from(LazyList((Block(startPos, startPos), List())), Set())
